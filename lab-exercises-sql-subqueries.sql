...
-- 1. Determine the number of copies of the film "Hunchback Impossible" that exist in the
-- 		inventory system.    
SELECT
...
-- 2. List all films whose length is longer than the average length of all the films in the
-- 		Sakila database.
SELECT
	title,
    length
...
SELECT
    payment.customer_id AS client_id,
    SUM(payment.amount) AS total_amount_spent
FROM payment
GROUP BY payment.customer_id
HAVING SUM(payment.amount) > (
    SELECT AVG(total_per_customer.total_amount_spent)
    FROM (
        SELECT
            payment.customer_id,
            SUM(payment.amount) AS total_amount_spent
